        ...
        {filtered.map((s) => (
          <div key={s.id} className="bg-card border border-border rounded-xl p-5 text-left transition-all hover:shadow-md hover:-translate-y-0.5 hover:border-blue-400/50 relative">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-11 h-11 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-sm font-bold flex-shrink-0 cursor-pointer" onClick={() => navigate(`/projects/student/${s.id}`)}>
                {s.name.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/projects/student/${s.id}`)}>
              ...
              <div className="flex items-center gap-1">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => navigate(`/projects/student/${s.id}`)}>
            ...
            <div className="flex flex-wrap gap-1 mb-3 cursor-pointer" onClick={() => navigate(`/projects/student/${s.id}`)}>
              {s.primary_areas.map((a) => <span key={a} className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-semibold">{a}</span>)}
            </div>
            <div className="grid grid-cols-2 gap-2 text-center cursor-pointer" onClick={() => navigate(`/projects/student/${s.id}`)}>
      ...
      <Dialog open={modal} onOpenChange={setModal}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2"><Label>{t('gifted.schoolRequired')}</Label><Input value={form.school} onChange={(e) => setForm({ ...form, school: e.target.value })} placeholder="Ex: FabLab Ipiranga" /></div>
            <div className="grid grid-cols-2 gap-3">
